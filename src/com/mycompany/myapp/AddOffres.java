 ...
 * @author bensa
 */
public class AddOffres extends BaseForm {
   OffresService ts = OffresService.getInstance();

    public AddOffres(Resources res) {
     ...
     this.setLayout(BoxLayout.y());
        ...
        //Widgets
         TextField DescTF = new TextField("", "Description");
             DescTF.setUIID("TextFieldBlack");
        TextField DesTF = new TextField("", "Destinatoin");
        DesTF.setUIID("TextFieldBlack");
        TextField nbPointTF = new TextField("", "Nombre de point");  nbPointTF.setUIID("TextFieldBlack");
        TextField pourcentageRedTF = new TextField("", "Pourcentage");     pourcentageRedTF.setUIID("TextFieldBlack");
        Button submitBtn = new Button("Ajouter");

        //actions
        submitBtn.addActionListener((evt) -> {
              if ((DescTF.getText().length()==0)||(DesTF.getText().length()==0)||(nbPointTF.getText().length()==0)||(pourcentageRedTF.getText().length()==0))
                    Dialog.show("Alert", "Please fill all the fields", new Command("OK"));
                else
                {
                    try {
                        Offres t = new Offres(DescTF.getText(),Integer.parseInt(nbPointTF.getText()),DesTF.getText(),Integer.parseInt(pourcentageRedTF.getText()) ); 
                        if( ts.getInstance().addOffres(t))
                        {
                           Dialog.show("Success","Offre effectuée",new Command("OK"));
                        }else
