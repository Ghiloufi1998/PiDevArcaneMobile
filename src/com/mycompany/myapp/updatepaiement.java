 ...
 * @author bensa
 */
public class updatepaiement extends BaseForm {
  PaiementService th = PaiementService.getInstance();


    public  updatepaiement(Paiement p,Resources res) {
...
//                
     ...
     //   Container x =  new Container(BoxLayout.x());
       ...
       // bindButtonSelection(myFavorite, arrow);
        ...
        this.setTitle("Modifier Paiement");
      ...
      Picker DateTF = new Picker();DateTF.setUIID("TextFieldBlack");
        TextField MontantTF = new TextField(String.valueOf(p.getMontant()), "Montant");MontantTF.setUIID("TextFieldBlack");
    //    TextField EtatTF = new TextField(p.getMode_Pay(), "Mode de paiement");DescTF.setUIID("TextFieldBlack");
        Vector<String> vectorGender;
        vectorGender = new Vector();

        vectorGender.add("Versement");
        vectorGender.add("Espéce");
         vectorGender.add("en ligne");
       ...
       // TextField statusTF = new TextField("", "Task's status [0 - 1]");
        ...
        submitBtn.addActionListener((evt) -> {
            if (th.updatePaiement(new Paiement(p.getID_PAi(),(Date)DateTF.getValue(), Integer.parseInt(MontantTF.getText()),gender.getSelectedItem()))) {
                Dialog.show("Success", "Paiement modifié", "retour menu", null);
            } else {
